...
경쟁사 5 곳의 리뷰 1.2만 건을 분석했습니다. 처음 가설 *(브랜드별 키워드 빈도가 다를 것)* 이 Chi-square 검정에서 p > 0.9 로 깨졌고, 그 자리에서 *제품 만족이 키워드보다 사용 경험·피부 타입에 더 의존* 한다는 시장 본질을 잡았습니다. 이 인사이트로 TF-IDF / LDA 토픽 모델까지 들어가 경쟁사 분화 축을 확인하고 3-beaty 의 진출 USP 를 정의했고, 그 결과를 성분-효과 지식 그래프 기반 GraphRAG 추천 챗봇으로 묶어 마케팅 부서가 *읽고 끝나는 보고서* 가 아니라 *질문하면 답이 돌아오는 시스템* 으로 활용할 수 있게 만들었습니다.
...
### 4-1. Amazon - 가설 깨짐에서 시장 본질, 그리고 USP 까지
...
### 4-2. TikTok - K-Premium Causal Robustness (selection effect 95% 발견)
