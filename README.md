...
신규 인디 브랜드 *3-beaty* (히알루론산 기반, Torriden 을 reference 로 한 가상 페르소나) 의 미국 Amazon 진출을 두 갈래로 풀었습니다.

**Amazon 갈래** — 경쟁사 5 곳의 리뷰 1.2만 건 분석. 처음 가설 *(브랜드별 키워드 빈도가 다를 것)* 이 Chi-square 검정에서 p > 0.9 로 깨졌고, 그 자리에서 *제품 만족이 키워드보다 사용 경험·피부 타입에 더 의존* 한다는 시장 본질을 잡았습니다. 이 인사이트로 TF-IDF / LDA 토픽 모델까지 들어가 경쟁사 분화 축을 확인하고 3-beaty 의 진출 USP 를 정의했고, 그 결과를 성분-효과 지식 그래프 기반 GraphRAG 추천 챗봇으로 묶어 마케팅 부서가 *읽고 끝나는 보고서* 가 아니라 *질문하면 답이 돌아오는 시스템* 으로 활용할 수 있게 만들었습니다.

**TikTok 갈래** — *K-Beauty 마케팅이 Amazon 매출로 연결되는가* 의 정량 검증. 약 4,600 영상을 반자동 수집한 뒤 정제·필터를 거쳐 56명 인플루언서의 1,680 영상으로 분석에 들어갔습니다. OLS → 매칭 기반 인과 추정 (PSM) → 같은 인플루언서 안에서만 비교하는 within-influencer Fixed Effect 까지 4 단계 회귀로 K-Premium 4.76 ~ 5.10 %p 를 추정했는데, 마지막 within-FE 단계에서 그 효과의 약 95% 가 *인플루언서 selection effect* 임이 드러났습니다. 마케팅의 핵심 레버가 *키워드* 가 아니라 *인플루언서 선정* 이라는 결론으로 넘어갔고, 이 selection 을 자동화하기 위해 추천 알고리즘을 ver.1 부터 ver.4 까지 진화시켜 1,540 조합 부트스트랩 검증에서 무작위 대비 1.25배에서 3.25배 ER% 까지 끌어올렸습니다.
