    ...
    try:
        ...
        sign_in_button = WebDriverWait(driver, 20).until(
            EC.element_to_be_clickable(
                (By.CSS_SELECTOR, "#nav-flyout-ya-signin a.nav-action-signin-button")
            )
        )
    ...
    try:
        ...
        next_page_button = wait.until(
            EC.presence_of_element_located(
                (
                    By.CSS_SELECTOR,
                    "li.s-list-item-margin-right-adjustment > span > a.s-pagination-next",
                )
            )
        )
...
def select_best_sellers():
    ...
    try:
        ...
        print(
            "Successfully selected 'Best Sellers' and waited for the next page to load."
        )
    except Exception as e:
...
def crawl_amazon(keyword="skin+care"):
    ...
    try:
        ...
        category_Department = wait.until(
            EC.presence_of_element_located((By.CSS_SELECTOR, "#n\\/3760911"))
        )
                    ...
                    for idx, item in enumerate(items):
                        try:
                            ...
                            if ASIN in ASIN_list:
                                ...
                                # 새 탭에서 상세 정보 열기
                                driver.execute_script(
                                    "window.open(arguments[0], '_blank');", item_url
                                )
                                ...
                                detail_bullets = driver.find_element(
                                    By.ID, "detailBullets_feature_div"
                                )
                                product_details = detail_bullets.find_elements(
                                    By.CSS_SELECTOR, "li span.a-list-item"
                                )
                                detail_dict = {}
                                for detail in product_details:
                                    try:
                                        ...
                                        key = (
                                            detail.find_element(By.TAG_NAME, "span")
                                            .text.split(":")[0]
                                            .strip()
                                        )
                                ...
                                brand = (
                                    driver.find_element(
                                        By.CSS_SELECTOR, "tr.po-brand .po-break-word"
                                    ).text
                                    if len(
                                        driver.find_elements(
                                            By.CSS_SELECTOR,
                                ...
                                feature_items = feature_bullets.find_elements(
                                    By.CSS_SELECTOR,
                                ...
                                # 텍스트를 추출하고 한 문자열로 합침
                                description = "\n".join(
                                    [item.text.strip() for item in feature_items]
                                )
                                print(description[:5])
                                ...
                                special_feature = (
                                    driver.find_element(
                                        By.CSS_SELECTOR,
                                        "tr.po-special_feature .po-break-word",
                                    ).text
                                    if len(
                                        driver.find_elements(
                                            By.CSS_SELECTOR,
                                ...
                                total_star = (
                                    driver.find_element(
                                        By.CSS_SELECTOR,
                                        ".a-popover-trigger .a-size-small.a-color-base",
                                    ).text
                                    if len(
                                        driver.find_elements(
                                            By.CSS_SELECTOR,
                                ...
                                for element in best_sellers_elements:
                                    if "Best Sellers Rank" in element.text:
                                        try:
                                            ...
                                            best_sellers_rank_text = element.text.split(
                                                ":"
                                            )[1].strip()
                                ...
                                if reviews == "No ratings":
                                    ...
                                    try:
                                        see_more_reviews_link = wait.until(
                                            EC.presence_of_element_located(
                                                (
                                                    By.CSS_SELECTOR,
                                            ...
                                            while review_count < max_reviews:
                                                ...
                                                for detail_review in detail_reviews:
                                                    try:
                                                        ...
                                                        customer_name = (
                                                            detail_review.find_element(
                                                                By.CSS_SELECTOR,
                                                                "span[class='a-profile-name']",
                                                            ).text
                                                            if len(
                                                                detail_review.find_elements(
                                                                    By.CSS_SELECTOR,
                                                                    "span[class='a-profile-name']",
                                                                )
                                                            )
                                                        ...
                                                        # 디버깅 출력
                                                        print(
                                                            f"Review {review_count} - customer_id: {customer_id}, customer_name: {customer_name}, review_title: {review_title}, review_rating: {review_rating}"
                                                        )
                                                        ...
                                                        print(
                                                            f"Error extracting review {review_count + 1}: {e}"
                                                        )
                                                        continue
                                ...
                                item_df["detail_dict"] = item_df["detail_dict"].apply(
                                    json.dumps
                                )
                                # price 및 total_star_mean을 숫자형으로 변환
                                item_df["price"] = pd.to_numeric(
                                    item_df["price"], errors="coerce"
                                )
                                item_df["total_star_mean"] = pd.to_numeric(
                                    item_df["total_star_mean"], errors="coerce"
                                )
