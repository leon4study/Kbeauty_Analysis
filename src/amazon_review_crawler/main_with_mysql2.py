...
# 기존 환경변수 구문 사용
DB_MASTER_HOST = os.environ.get("DB_SERVER_HOST")
DB_REPLICA_HOST = os.environ.get("DB_REPLICA_HOST")  # replica가 있다면 설정

# DSN 만들기 (mysql+mysqlconnector)
master_url = f"mysql+mysqlconnector://{DB_USERNAME}:{DB_PASSWORD}@{DB_MASTER_HOST}:{DB_PORT}/{DB_DATABASE}"
replica_url = None
if DB_REPLICA_HOST:
    replica_url = f"mysql+mysqlconnector://{DB_USERNAME}:{DB_PASSWORD}@{DB_REPLICA_HOST}:{DB_PORT}/{DB_DATABASE}"
...
def get_asin_from_sql():
    ...
    설명: processed_flag 컬럼을 두어 이미 저장된 ASIN을 필터링하면 스크립트 내 중복 체크 비용을 줄임.
    """
    try:
        query = "SELECT ASIN FROM items WHERE processed_flag = 1"  # processed_flag로 이미 처리된 건 표기 권장
        # replica 우선 사용
        df = my_sql_client.fetch_as_dataframe(query, use_replica=True)
        return df['ASIN'].astype(str).tolist() if not df.empty else []
    except Exception as e:
