...
def load_settings():
    ...
    )  # 현재 파일(chatbot.py)의 디렉토리
    yaml_path = os.getenv(
        "GRAPHRAG_CONFIG", os.path.join(base_dir, "indexing", "settings.yaml")
    )
    ...
    if not config:  # 설정이 비어 있으면 경고
        print(
            "⚠️ WARNING: settings.yaml 파일이 비어 있거나, 올바르게 로드되지 않았습니다."
        )
    ...
    return {
        ...
        "community_level": int(
            os.getenv("COMMUNITY_LEVEL", config.get("community_level", 2))
        ),
        ...
        "embeddings_api_base": os.getenv(
            "EMBEDDINGS_API_BASE", config.get("embeddings_api_base")
        ),
...
def gradio_interface(settings):
    ...
    with gr.Blocks() as demo:
        # 헤더 및 설명 추가
        gr.Markdown("# 맞춤 화장품 추천")
        gr.Markdown(
            "**화장품 성분에 대해 궁금한 내용을 입력하면, 관련 정보를 제공합니다.**"
        )
        ...
        with gr.Row():
            query_input = gr.Textbox(
                label="질문을 입력하세요",
                placeholder="예: 화장품 성분에 대해 설명해줄래?",
                lines=2,
            )
        ...
        # 버튼 클릭 시 쿼리 실행
        query_button.click(
            fn=lambda x: gradio_query(x, settings),
            inputs=query_input,
