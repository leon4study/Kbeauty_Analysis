#YAML 파일을 기반으로 RAG (Retrieval-Augmented Generation) 시스템을 설정
#하려면 몇 가지 핵심적인 설정을 조정하고, 이를 활용해 Embedding과 LLM을 연결해야 합니다.
#주어진 설정은 이미 임베딩 모델과 LLM(언어 모델)을 정의하고 있으며, 이를 통해 RAG 시스템을 구축할 수 있습니다. 아래 단계별로 필요한 작업을 안내드리겠습니다
